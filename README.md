...
- **Continue a conversation from the app**, in either of two ways — Settings → *Send prompts from the app*, on by default. Nothing is spawned until you press a button or type a prompt.
  - **An embedded terminal** (the default): the real Claude Code CLI, running in the page where the composer's box would be, with everything the TUI does — slash commands, plan mode, `/model`, colours and all. Drag its top edge to resize it, `⤢ full screen` to give it the window, and it **keeps running while you read other sessions**: the pseudo-terminal belongs to the server, so closing the tab loses the view and nothing else. Typing `/exit` closes the panel; a CLI that dies badly leaves its last screen up, because that screen is the diagnosis.
  - **A composer** (experimental — everything it draws, it draws itself): a box at the foot of the conversation that picks the model, the effort and plan mode, puts Claude's questions to you with their drawings, notes and free text, and lets you approve a plan or send it back with a note, passage by passage.

  Either way the answer appears in the viewer as Claude Code writes it, and either way it is blocked while the session is open in a terminal elsewhere: two processes writing one transcript corrupt it. The conversation runs to the foot of the window with the box riding inside it, so nothing is left hidden behind it when it grows.
- **Start a new one from the app** (`+ New` in the header, behind the same switch): pick the project from the same list the filter sidebar draws — same dots, same colours, same counts — or point at any folder on the machine, typed or chosen through the system's own folder browser (that button is local-only; typing the path works from anywhere). The page you send the first prompt from is the session view a moment early — same width, same zoom, the same composer at the foot — and the id is minted here, so the page is already standing where the transcript will land: the moment Claude Code writes the file, it hands over to the ordinary viewer and gets out of the way. Model, effort and plan mode are all there on that first prompt, so a session can start straight into planning.
