...
- Live updates via SSE — an open session wears a LIVE badge counting how long its process has been there, beside a state badge of its own: `working` with a turning ring, `waiting` in amber with the CLI's own words for what it wants on the hover, `idle` with a hollow dot. Its row also counts the time since its last activity by the second — hours at most, never days.
