...
- **Mentioned**: the third file question — what a session only *talked* about. The paths its answers named, that are really on disk, each with the spelling it was written in, a click that opens the file and a jump to the sentence it came from. Most of what an answer names is a partial path or a placeholder, so the panel is mostly a filter and says plainly what it dropped and why — and its count appears only once it has been checked, because a number that promises fourteen rows and draws one is worse than no number.
