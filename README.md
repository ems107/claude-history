...
- Live updates via SSE — an open session wears a LIVE badge counting how long its process has been there, beside a state badge of its own, each with its own clock: `working` with a turning ring times the whole turn (a dialog answered mid-turn does not restart it), `waiting` in amber — the CLI's own words for what it wants on the hover — and `idle` with a hollow dot time how long they have stood. All the clocks tick by the second, in hours at most, never days.
