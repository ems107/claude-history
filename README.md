...
- **Hunk-level staging**: each hunk of a diff has its own stage and discard, so one file with two unrelated edits can contribute just one of them to a commit. And a conflicted file shows the three versions git is holding — ours, theirs, and what they both started from — instead of a diff full of merge markers.
