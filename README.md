...
- **Mentioned**: the third file question — what a session only *talked* about. Paths its answers named (and its subagents' reports named), that are on disk and in neither other panel, each with the spelling it was written in and a jump to where it was said. Most of what an answer names is a partial path or a placeholder, so the panel is mostly a filter and says plainly what it dropped and why — its count appears only once it has been checked, because a number that promises fourteen rows and draws one is worse than no number.
