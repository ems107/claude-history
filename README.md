...
- **Remotes**: fetch (with prune), pull and push, carrying the ahead/behind counts on the buttons. Pull refuses to invent a merge commit and offers rebase or merge beside the refusal; push shows the command it will run, and force is always `--force-with-lease`, which still refuses if the remote moved since your last fetch. If git needs credentials it fails in under a second with a button that opens a terminal where you can store them, rather than hanging.
- **The rest of git**: rebase, cherry-pick, revert, reset, stashes, tags and worktrees, each from the place it belongs — the sidebar, or the commit you are looking at.
- **Hunk- and line-level staging**: each hunk has its own stage and discard, and individual lines can be picked with a click (shift-click to reach), so one file with three edits can contribute one. Staging lines only ever writes what the next commit will contain — never the file on disk — so it is undone with a single unstage. And a conflicted file shows the three versions git is holding — ours, theirs, and what they both started from — instead of a diff full of merge markers.
