...
- Minor UI state lives in the browser, not in files: `localStorage` (thinking toggle, sidebar width, the inspector's width, whether the header's `more` row is open, the embedded terminal's height — properties of this screen rather than of the machine — and, per session, whether you left its terminal collapsed) and per-tab `sessionStorage` (list filters/scroll for back-navigation, and which message you had selected in a conversation). Active filters are also reflected in the URL.
