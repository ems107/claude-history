...
- **Continue a conversation from the app**, in either of two ways — Settings → *Send prompts from the app*, on by default. Nothing is spawned until you press a button or type a prompt.
  - **An embedded terminal** (the default): the real Claude Code CLI, running in the page where the composer's box would be, with everything the TUI does — slash commands, plan mode, `/model`, colours and all. Drag its top edge to resize it, `⤢ full screen` to give it the window, `⌄ minimise` to leave it as its own title bar — one line, CLI still running — when you want the conversation back. The height is remembered per window and the collapsed state **per session**, so a conversation you left tidied away comes back that way and every other one comes back open; starting a terminal always opens it. It **keeps running while you read other sessions**: the pseudo-terminal belongs to the server, so closing the tab loses the view and nothing else. It also **brings its own pseudo-console** rather than using the one in your Windows, so it behaves the same on Windows 10 as on 11 — that component differs on every build, and on an older one it cost the alternate screen, the mouse, Shift+Enter and pasting more than one line. Typing `/exit` closes the panel; a CLI that dies badly leaves its last screen up, because that screen is the diagnosis.
