...
/**
 * The two buttons that do not undo themselves — at the foot of *This instance*,
 * which is what they act on.
 *
 * They were a `flex-wrap` beside *Open data folder* once: the same row, the same
 * size, a red border the only thing between "show me a folder" and "remove the
 * application". The first fix was an area of their own, and that went too far
 * the other way — a whole destination in the rail for two buttons, exiled from
 * the block that names the very install they remove.
 *
 * A marked subsection is what both of those were reaching for: the rule and the
 * red heading separate them from the benign buttons above, each one says in a
 * line what it costs, and they are still where you would look for them. Both are
 * local-only (`stopServer`, `uninstall`), so over remote access they grey
 * together with one reason each rather than scattered through the group.
 */
