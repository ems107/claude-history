    ...
    <div className="text-xs">
      ...
      {w.resetsAt && (
        <div className="mt-0.5 text-[11px] text-[var(--text-dim)]">
          resets {formatDateTime(w.resetsAt)} ({relativeTime(w.resetsAt)})
        </div>
      )}
    </div>
  );
          ...
          <>
            {five && (
              <span className="flex items-center gap-1">
                <span className="opacity-70">5h</span>
                <Bar pct={five.utilization} className="h-1.5 w-10" />
                <span className="font-mono">{Math.round(five.utilization)}%</span>
              </span>
            )}
            {week && (
              <span className="flex items-center gap-1">
                <span className="opacity-70">wk</span>
                <Bar pct={week.utilization} className="h-1.5 w-10" />
                <span className="font-mono">{Math.round(week.utilization)}%</span>
              </span>
            )}
          </>
        )}
        ...
        <>
          ...
          <div className="absolute right-0 z-50 mt-1 w-72 rounded-lg border border-[var(--border)] bg-[var(--bg-raised)] p-3 shadow-xl">
            ...
            <p className="mt-3 text-[10px] leading-snug text-[var(--text-dim)]">
              Same figures as Claude Code’s /usage. Read from your stored session, never modified; refreshed at most every
              5 minutes{data.fetchedAt ? ` · last ${relativeTime(data.fetchedAt)}` : ''}.
            </p>
          </div>
        </>
      )}
