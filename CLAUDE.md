...
- Records: `{t, lvl, src, pid, v, msg, data?, err?}`. `t` is local ISO **with offset** — still sortable and `Date.parse`-able, but readable without doing timezone arithmetic by hand. `pid` and `v` are on every record: two instances sharing a day's file should be impossible (one port), and this is what makes it obvious when it is not.
