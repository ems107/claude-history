...
| [The GIT tab](docs/AI_GIT.md) | the one feature that writes outside this app: the runner, the credential lockdown, the graph, staging by hunk and by line, the bin |
...
| `.\dev.ps1` | **start the dev instance** on `http://127.0.0.1:7434` (builds if needed, detached, opens the browser). `-Build` `-Restart` `-Stop` `-Foreground` `-Seed`, and `-Remote` to bind wide WITHOUT asking the firewall. A phone reaches it the ordinary way instead — remote access on in its own Settings, its own port opened ([Remote access](docs/AI_REMOTE_ACCESS.md), [The phone](docs/AI_MOBILE.md)) |
...
scripts/        package.mjs · release.mjs · git-fixture.mjs (the bench the GIT
                tab is verified against — never a real repository)
```

## Hard rules

- **The app only READS `~/.claude`.** Never write, create or lock anything inside it — `.credentials.json` included. Our writes go to the cache dir, `userdata.json`, its `backups\`, `logs\` and `git-undo\`, and nowhere else. → [Architecture](docs/AI_ARCHITECTURE.md)
- **The GIT tab is the one thing that writes outside all of that, and only where you sent it.** A checkout or a commit changes a folder of the USER's, on an explicit action in that tab and never on its own — nothing else in the app has a verb that reaches a repository. Discarding is the only thing in it git cannot undo, so the bytes go to `git-undo\` first: beside `userdata.json` and never in the cache, because "Clear cache" must not be a way to lose work. → [The GIT tab](docs/AI_GIT.md)
- **And the GIT tab is NEVER exercised against a real repository — not a write, and not a read either.** `node scripts/git-fixture.mjs --reset` builds the bench under `%TEMP%\claude-history-git-fixture`, and a **second** instance serves it on port **7436** with its own `--data-root`, cache, logs and `userdata.json` (7433 is the release, 7434 the dev instance, 7435 `preview.ps1`). Every clone on this machine is the user's working state, including this one: a fetch is a network call somebody did not ask for, a status read races the terminal they have open beside it, and there is no version of "I only looked" that is worth the risk of having been wrong about which verb was about to run. The fixture is also the only place a check can assert exact numbers, because it is built to a fixed clock. **Real repositories are the user's to drive by hand** — if something can only be shown there, say so and hand them the command. → [The GIT tab](docs/AI_GIT.md), [Verifying a change](docs/AI_TESTING.md#the-git-tab)
- **Three folders of Claude Code's are read, and a fourth costs an argument.** `~/.claude` is the transcript; `%TEMP%\claude\…` is the scratchpad; `%LOCALAPPDATA%\claude-cli-nodejs\Cache\…` is the MCP logs, the only place that says WHY a server failed. Each is undocumented and each may change under us, so a new one needs a reason the other three cannot serve, a read that is **lazy and its own endpoint**, and "it is not there" drawn as an ordinary state. All three are addressed by the index's own `encodedDir` — Claude Code slugifies a cwd once and spells it identically in all of them, so **nothing re-implements that encoding**. → [Transcripts](docs/AI_TRANSCRIPTS.md)
- **Every automatic network call is switchable off, and two exist today**: the update-availability check (a conditional GET to `api.github.com`, downloads nothing) and the subscription-usage read. Everything else is user-triggered: fetching prices (`POST /api/prices/fetch` scrapes `platform.claude.com/docs/en/about-claude/pricing.md` — there is no pricing API; preview only, nothing persists until saved, and the parser fails loudly so the UI can fall back to manual editing), applying an update (confirmed, SHA-256 verified), the auto-reload's "Send it now", and the GIT tab's `fetch`/`pull`/`push` — the first ones that reach a **third-party host the user configured**, which is why a remote must be one the repository already has and why the `.git` watcher may never lead to a fetch. **A third automatic one may be added, and costs four things**: a reason a user-triggered call could not serve, its own switch in Settings, OFF by default, and a line in the README. The count is not the rule — the guarantee behind it is, and it is that nothing here reaches the network behind the user's back. The easiest way to break THAT is still by accident, with a voice: the notification narrator offers only voices with `localService`, because Edge's "Natural" ones are synthesised on Microsoft's servers. → [The viewer](docs/AI_VIEWER.md)
...
- **The wide bind is earned, never assumed** — an instance listens on the network only with the switch on, credentials set and the Windows Firewall already allowing the port, because a listen with no rule is what makes Windows raise its "allow this app?" dialog. **Nothing this app does on its own may make Windows ask for permission**; the only dialog allowed is the UAC of the firewall button. **A dev instance earns it exactly as a release does** — its own port, its own rule, its own switch — and `--host` is the one thing that skips the gate rather than passing it. The wide bind is still only safe because **a request from another machine gets nothing until it signs in** — those two remain one feature. → [Remote access](docs/AI_REMOTE_ACCESS.md)
- **Every state-changing request must come from our own pages** (403 otherwise). A path or a cwd never comes from the request — it comes from the index, with exactly two named exceptions, each of them a folder the user is naming for the first time and no index could hold: the one a new session is started in, and the one added to the GIT tab as a repository or a scan root. Everything downstream of that second one travels as `GitRepo.id` and is looked up, so the exception ends where it began. → [Architecture](docs/AI_ARCHITECTURE.md)
...
- **Nothing that ends this server or changes how prompts are sent may run while the app is running Claude** — stop, restart, update, clear cache, restore userdata and the two chat settings all refuse while a CLI of ours is alive, **idle ones included**, and the 409 carries the list so the dialog can offer to close them. The four that END this process refuse for a second reason as well: a repository being written to, which nothing left running could finish. → [Running Claude](docs/AI_RUNNING_CLAUDE.md)
- **Every git process starts in `runGit()` and nowhere else.** The flags, the credential lockdown and the command panel's recording all live inside it, so a call site that assembles its own is invisible in the panel and can HANG for ever waiting for a password nobody can type. → [The GIT tab](docs/AI_GIT.md)
...
- **A session that never became a conversation is nobody's to leave running**, in either door: no transcript means no list, no search, no badge and no URL anyone has, so a CLI on one is closed a minute after the last browser leaves. Everything WITH a transcript keeps its own rule — the terminal no timeout, the composer the cache's hour. → [Running Claude](docs/AI_RUNNING_CLAUDE.md#a-session-that-never-became-a-conversation)
...
- **A phone is a supported browser, and the desktop is what must not change for it.** One threshold in three spellings (`max-md:`, `@media (width < 48rem)`, `MOBILE_QUERY`), nothing revealed on hover as the only way to a feature, no `title=` as the only explanation, nothing that can only work on the machine drawn at all (`useHideLocalOnly` — the server still refuses all fourteen), and no route that scrolls the document sideways at ANY width — 768 to 1020 is where the desktop layout is drawn in a window no desktop is, so the rows of the frame wrap there rather than overflow. **And the threshold is crossed live**: a window dragged narrow and a phone turned sideways are the same event with no reload in it, so `useIsMobile()` may decide what is rendered and never whether a hook is CALLED. → [The phone](docs/AI_MOBILE.md)
...
- **A hidden project is unbrowsable, not unreachable.** Everything that browses goes through `visible()` and `index.get(id)` never does; anything that is not browsing asks `findProject` or `projectsAll()` — the GIT tab's discovery is the second — because a folder you chose not to read in a list is still one you may start a session in, or work in. → [Architecture](docs/AI_ARCHITECTURE.md#a-hidden-project-is-unbrowsable-not-unreachable)
...
There is no automated test suite: this is a personal tool and it is checked against real data. [docs/AI_TESTING.md](docs/AI_TESTING.md) holds the 70 checks, grouped by area and referenced by number from the other documents, plus the fixture survey — **the session ids used as fixtures expire**, so start there rather than trusting an id you read elsewhere.
