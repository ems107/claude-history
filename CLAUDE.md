...
- `pnpm package -- --version X.Y.Z` — build the portable zip (`scripts/package.mjs`): esbuild-bundles the server to `server.cjs` (CJS — `main.ts` must stay free of top-level await), embeds a pinned Node runtime, and assembles the versioned layout + installer scripts into `dist/`. Without `--release` (which only `release.mjs` passes) the version gets a **`-dev` suffix**, so a hand-built zip can never claim to be a published release. That matters beyond cosmetics: an install reporting a bare `1.3.1` would consider the real 1.3.1 release "already installed" and never offer it; `1.3.1-dev` sorts below `1.3.1` and is superseded normally.
