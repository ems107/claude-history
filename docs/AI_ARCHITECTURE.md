...
- **Our writes go to exactly five places**: the cache dir, `userdata.json`, its `backups\`, `logs\` and `git-undo\`.
- **The GIT tab is the one feature that writes anywhere else**, to a repository the user listed for it and on an explicit action there ([AI_GIT.md](AI_GIT.md)).
...
- **A path or a cwd never comes from the request** — it comes from the index, or is composed from it. Two exceptions, both named below: the folder a new session is started in, and the folder added to the GIT tab.
...
- **What the browsing views may show goes through `visible()`**, and `index.get(id)`, `findProject` and `projectsAll()` never do — a hidden project is unbrowsable, not unreachable.
...
**A session that has just grown is re-summarized without its enrichment, and keeps the last one anyway.** The write invalidates the cached entry and the full parse runs in the background, so the figures are **~105 ms** behind the file, once per message, on a live session's own transcript. That is the pipeline working as intended. What must not happen is anything downstream *seeing* that gap, so `refreshSummary()` carries the previous `enrichment` onto the new summary — along with `descendants`, the other field a fresh summary blanks and nothing restores in time (only a CHILD being re-enriched puts it back, which may never happen). Figures one message stale for a tenth of a second, instead of none at all. **`enrichment: null` now means exactly one thing: this session has never been enriched.**

It is a contract worth its weight, because the gap was drawn every time. Losing the figures dropped a row out of the session header, which moved the whole page twice per message ([AI_VIEWER.md](AI_VIEWER.md#nothing-above-the-conversation-may-change-height)); in the list it took the cost, the compaction count and the PR and fork badges out of a live session's row, so everything to their right — the LIVE pill and the WORKING spinner among them — slid sideways and back every couple of seconds. Sorting and filtering read the same field: the same gap threw the row to the end of a cost-sorted list and out of the `pr` and `fork` filters entirely. One field, four symptoms, and the only place that can fix all four is the one that blanks it.
...
| How much of each session has been READ (the row's unread count) | in memory, `core/readMarks.ts` | **no — and deliberately**, same argument |

**The last two rows are the ones that could have been persisted and must not be.** A notification is raised by a TRANSITION — a session seen to leave `busy` — and a restart loses the transitions along with the rows, so a saved row would outlive its own evidence and claim to have watched something happen that nobody watched. Emptying on restart is the honest behaviour, and it also keeps this out of the five places in `index.ts` that a new kind of user data has to be added to at once. **A read mark is the same fact about the other side of the conversation** and is kept the same way, in this process and nowhere else: a mark on disk would claim tomorrow to know what somebody had read, when the only thing that ever made that true was this process watching it happen. It is deliberately NOT in the browser either — that was the first attempt, and a reload lost the count while the bell beside it on the same row came straight back ([AI_VIEWER.md](AI_VIEWER.md#what-a-list-row-says-you-have-not-seen)).
...
- **The GIT tab writes DIRECTLY, and it is the only thing here that does.** Not to `~/.claude` and not to anything of ours: to a repository of the user's, on a button they pressed in that tab. It is the one place where the containment above stops being the whole story, so it carries its own set of rules instead — the single runner, the credential lockdown, the id that is the only thing a request may carry, and the copy taken before anything is discarded. → [AI_GIT.md](AI_GIT.md)
...
- **Enumerating a directory is the second thing it reads on its own initiative, and it is contained differently.** `GET /api/sessions/:id/scratchpad` lists a session's temp working folder, which no transcript has to have named — so where the file viewer takes a reference and resolves it, this takes **nothing at all** and composes the root itself from `os.tmpdir()`, the session's `encodedDir` and its id ([why that name is already in the index](AI_TRANSCRIPTS.md)). Three things pay for the exception and must survive any change to it: the client cannot name a path, the walk is **capped** (`MAX_SCRATCHPAD_ENTRIES`) so a listing cannot become a filesystem scanner, and the GET carries **its own `isSameOrigin`** like the two beside it. A swept folder is a 200 with `exists: false` — the ordinary end of a scratchpad, not a failure. → [AI_VIEWER.md](AI_VIEWER.md#the-scratchpad-is-the-fourth-file-question)
...
- **`POST /api/git/repos` is the other one**, and it ends where it begins: a repository or a scan root is a folder no index could know about until somebody names it, so it is validated rather than trusted (absolute, existing, a directory, and stored as the work tree's top level rather than as what was typed). Everything downstream travels as `GitRepo.id` — a hash of that resolved key — and `repoOf` in `routes/git.ts` is the only place it becomes a path again, which is what makes "never take a path from the client" structural here rather than a check somebody has to remember in twenty handlers. → [AI_GIT.md](AI_GIT.md)
...
- **A turn in flight refuses `POST /api/server/stop`, `POST /api/server/restart`, `POST /api/uninstall` and `POST /api/update/apply` with 409**, the same way an update in flight already refused the others — and so does a repository being written to, for the same reason one step over: this process is the only one that could finish the rebase it started.
- **Network policy**: two automatic calls exist today — see the summary in [CLAUDE.md](../CLAUDE.md#hard-rules), the update check in [AI_DISTRIBUTION.md](AI_DISTRIBUTION.md) and the usage read in [AI_RUNNING_CLAUDE.md](AI_RUNNING_CLAUDE.md). A third is allowed on the terms written there: a switch, off by default, and a line in the README. The GIT tab's `fetch`/`pull`/`push` are user-triggered and therefore outside that count, but they are the first calls this app makes to a host the USER configured — so the remote must be one the repository already has, and the `.git` watcher may never lead to a fetch ([AI_GIT.md](AI_GIT.md)).
...
[AI_TESTING.md](AI_TESTING.md) — checks 1, 5, 8 (hiding), 21 (the files endpoint), 25 (starred messages), 28 (the image endpoint), 48 (the scratchpad listing), 61 (the other folder that comes from a request), and the same-origin cases in 19.
