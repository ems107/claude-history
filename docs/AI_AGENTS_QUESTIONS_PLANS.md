...
- **This must not be hung off the enrichment.** The enricher streams every one of these files already and could collect the `<task-id>`s, closing the nested case — but enrichment lands *late by design*: a grown session is re-parsed in the background, ~105 ms per message, and its summary keeps the PREVIOUS figures meanwhile ([AI_ARCHITECTURE.md](AI_ARCHITECTURE.md)). That is what stops the counts flickering, and it is exactly why an indicator cannot be hung off them — it would be a message behind for as long as the parse takes, saying an agent is idle while it writes. An indicator that lags lies as surely as one that flickers. The clock it does use is `SubagentMeta.lastWriteMs`, the transcript's mtime, served with the metas and refreshed by the same event.
