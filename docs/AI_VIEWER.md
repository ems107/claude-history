...
- **The jump STEPS**: `↑ 1/4` goes to the first naming, `↑ 2/4` to the second, and the fourth press comes back round to the first — the label is a promise about the press, so it names the destination. The cursor is local to the row, so closing the panel starts again at the first. Exact by construction: it walks `row.messages`, which is the same list `×N` counts.
