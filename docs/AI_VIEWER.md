...
- **Who gives up the pill's corner depends on what is in it.** The composer keeps `Send` out of it with a `max()` over the column width and gives up nothing else, because a composer has spare corner to spare. A terminal does not: every cell is content, and reserving 120 px there just makes the panel narrower than the conversation above it for no reason a reader can see — at `Full` width, 120 px of a 1348 px panel. So for a terminal the PILL moves instead (`liftPx`, from the panel's measured height), and the panel keeps the whole column.
