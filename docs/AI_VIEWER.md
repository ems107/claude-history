...
- **The jump leaves the find bar open, and that is where "the other three" is answered.** `↑ the mention` sets the anchor and the marks, then `setQuery` + `openBar(true)`: the `All` scope, so a naming inside a folded stretch counts too, with the input focused and its text selected so typing replaces it. Enter steps. Two rules, both learnt the hard way: the query is set **before** `openBar`, or `openBar` seeds the deep link's own terms over it; and it is **one term, the basename**, because the bar searches in `'phrase'` mode always — handing it `<path> <basename>` searches for that literal sentence, which appears nowhere, and the full path alone would only ever find its own spelling. Building a second "next occurrence" in the panel would be two implementations of one idea, and they would disagree. The bar counts what the bar counts (tool calls included), so its total is normally larger than `×N`, which counts namings in the answers.
