...
The third is the weakest of them by nature and is built to admit it, because **a path in prose is written for a person**: `core/git.ts` for the real thing, `<pid>.json` for a naming scheme, `~/.claude` for a folder, `vX.Y.Z` for nothing at all. Measured over five sessions, 14 of 64 such paths resolved to a file that is really there — so the panel is mostly a filter, and what it drops it counts and explains on screen (`980751cb`: 151 named, 14 listed, 118 that nothing is at, 19 already in the other two panels).
...
- **Only where a path is a link today**: an assistant `text` block, and a subagent's report inside a notice (both go through `Markdown`). **A prompt is deliberately not read** — it renders `whitespace-pre-wrap`, so a path typed into one is not a link anywhere in this app. In practice the reports dominate: 23 of 23 rows in one session here, which is also why the origin is stated once in the heading rather than as a chip on every row.
- **Deduplicated on the RESOLVED path, which is the only identity a mention has.** Two reports naming `server/src/core/parser.ts`, one absolutely and one relatively, are one file and drew two rows until the dedupe moved there — and it can only happen after the stat, because resolution is the server's answer. `×N` absorbs the spellings.
...
- **No disk column here**, unlike the sent panel: existing is the entry price rather than a finding. The size and the date are the file's own.
