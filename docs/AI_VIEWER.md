...
**The fold strip carries the turn's settled duration** — `3 responses · 12 tool calls · 2 min 45 s`, folded and unfolded alike, the exact start → end on the hover. The figure is `turnSpan` (`lib/turnActivity.ts`, pure): first kept item to the newest stamp anything in the turn carries, tool results included, which is **the same boundary the working row's `total` counts from live** — so the strip settles on the number the row was showing when the turn ended. It is deliberately not the CLI's own `turn_duration` line, which stays dropped ([AI_TRANSCRIPTS.md](AI_TRANSCRIPTS.md#system-lines-by-subtype) for the measurements). Two rules keep it honest: **the turn in flight holds it back** (`TurnView inFlight`, fed by `TurnList lastTurnInFlight` beside the footer plumbing — busy or waiting on the page, `running` in the drawer), because a second figure that stops at the last write would quietly disagree with the live clock underneath; and it is **a duration, never a datetime** — a date reappearing on the strip is check 17's failure signal for notice-opened turns, and this must not read as one. A turn with no strip (nothing of the assistant's to fold) shows no figure, and that is a turn Claude never answered.
