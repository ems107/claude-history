...
- **Nothing here is a one-way door.** The tab's own hidden list (`gitHidden` in `userdata.json`, and not the project one `visible()` reads) is written from the picker's `Hide` and read back in *Settings › Git*, which lists what is hidden and offers each one back. The server had always accepted `hidden: false` and nothing had ever sent it, so a mis-tap meant editing `userdata.json` by hand — the shape of fault to look for is a verb with no inverse anywhere in the UI, not a missing endpoint.
