...
**It is a VIEW, not a dock.** It had a drag handle at the foot of the page and competed for height with a graph pane that has a fixed one, so pulling it up pushed the commit's detail off the bottom of the window. It now takes the whole area under the toolbar — a third view beside Commits and the working tree, and mutually exclusive with them, so none of the three may look chosen while another is on screen. The toolbar stays above it: the reason to open this is usually to watch what the next Fetch does. On a phone it is unchanged, a sheet behind the `⋮`.
