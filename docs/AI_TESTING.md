...
| Files a session only MENTIONED | `980751cb` (151 named, 14 listed — the filter's arithmetic), `1806cedb` (the same file named absolutely and relatively, so the resolved-path dedupe) | — |
...
- **The `Mentioned` panel is the index of these references, and it is mostly a filter** ([AI_VIEWER.md](AI_VIEWER.md)). The one rule to check first is that its collector is **never looser than the renderer**: for any session, every path it lists must also be a link in the messages, and the reverse need not hold. Then the arithmetic, which is the point of the panel — `980751cb` names 151 paths and lists **14**: 118 that nothing is at, 19 already in Changed or Sent Files, and every dropped one accounted for in words on screen. **No silent caps**: past `MAX_STAT_PATHS` the panel must say how many were never checked.
- **Its count is the one count in the header that is not a transcript fact**, so: the button opens reading `Mentioned` with **no number** and makes **no** `/api/files/stats` request until pressed; after one press it reads `Mentioned (N)` where N is exactly the rows drawn — check both, in that order, on a session with many candidates. `Mentioned (0)` must still open and explain itself in words rather than draw an empty box.
- **One row per FILE, never per spelling.** `1806cedb` is the fixture: its reports name `server/src/core/parser.ts` both absolutely and relatively, which drew 23 rows for 19 files until the dedupe moved onto the RESOLVED path — the only identity available, and only after the stat comes back. No filename may appear twice, and `×N` must have absorbed the repeats. The `as written` column must show the **relative** spelling only; an absolute one belongs in the folder tail and the title, and printed in the row it pushed every row past the window.
- **A mention usually lives in a subagent's report** (23 of 23 rows in `1806cedb`), which is a notice and not a bubble — so `↑ the mention` flashes the notice itself, and an assertion scoped to `[data-bubble].match-flash` fails on a working jump. Where every row shares an origin the panel says so once in its heading instead of putting the same chip on every row.
