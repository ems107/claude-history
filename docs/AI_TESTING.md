...
- **The jump must leave a find bar that WORKS**, which is the assertion that caught the first attempt: after one press the bar must be open, its query must be a **single term with no space in it** (two terms joined by one is a phrase that matches nothing — the bar is always in `'phrase'` mode), it must report a non-zero count, its input must hold the focus with the text selected, the scope must be `All` (open the bar's options panel to read it), and pressing *Next* must give a `find-current` range without disturbing the query. `sessionFiles.ts` in `1806cedb` reads 65 matches on `All` against `×2` in the panel — the bar counts tool calls too, and that difference is correct, not a bug.
