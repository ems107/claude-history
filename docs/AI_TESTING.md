...
- **The browser half over CDP** (check 26's harness): the header must offer `+ New` — and must NOT while `chatEnabled` is off — the picker must list the projects plus `Another folder…`, a relative path must be refused **in words beside the input with the page staying put**, and a real folder must bring up the composer. Then send: `sending…` must appear, `[role="status"]` must light, and the page must reach `/session/<id>` **on its own**, with the answer drawn from the transcript, `[data-bubble]` ≥ 2 and the viewer's own composer at the foot (`[data-sticky-bottom] textarea`). Never leave the page before a prompt goes out — a handover that fires on an empty box is a bug, and the only way to see it is to sit on the picker with a CLI open.
