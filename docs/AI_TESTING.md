...
- **The folder browser** (`POST /api/pick-folder`): the button must sit beside the typed-path box and open the system dialog on the server's desktop — a visible top-level window owned by pwsh, titled `Select Folder` (`Browse For Folder` under Windows PowerShell 5.1, which is the older dialog and still correct). Closing it must answer `{"path": null}` and leave the typed path alone; choosing a folder must put it in the box. From another machine the BUTTON must be disabled with its reason while the box beside it still works — that split is the whole point. **Do not drive this dialog with UI Automation**: the first `Edit` in its tree is the file list's inline rename box, not `Folder:`, and setting it renames a folder on the user's disk (Windows refused the name, which is the only reason that cost nothing). Check the window exists and close it with `WM_CLOSE`; the encoding half is checked without a GUI, by writing a path containing `ñ` through the same `WriteAllText` and reading it back in Node.
