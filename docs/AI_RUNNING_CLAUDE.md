...
- **Collapsing it is not closing it.** *minimise* leaves the panel as its own title bar — 32 px, one line — with the host `display: none` rather than unmounted (unmounting takes xterm's DOM with it, the same trap full screen avoids by being a class), the CLI running and the socket attached. It is what "drag it smaller" cannot be: the height has a floor, because under `TERMINAL_HEIGHT_MIN` there is no room for a status line and a prompt, so getting the conversation back needed its own gesture. `refit()` runs on the way back because xterm has been measuring a box of zero. **Full screen expands it first**: filling the window is a request to see the thing, and a full screen with a hidden host is a blank one.

  **Remembered per SESSION, unlike the height**, and the difference is what each one is about: a height is a property of this window, which a phone and a 4K monitor answer differently, while collapsing the panel is something done to one conversation — to read it with the terminal out of the way — and coming back to that conversation should find it as it was left. As one switch for all of them (`terminalMinimised` as a bare `true`), collapsing one made the next terminal started anywhere else come up collapsed, which is a panel hiding itself for a reason nobody can see: reported as real and unreproducible, because the cause was in another session. It is now a capped list of the ids that are collapsed, absence being the default, so nothing accumulates for the ordinary case. **Starting a terminal always expands it**, whatever that session's last state was — pressing the button is asking to see the thing — and the legacy `true` reads as an empty list, which is the one setting whose memory had to go.
