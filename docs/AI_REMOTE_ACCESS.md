...
Two facts to keep the risk in proportion. Every rule this app actually creates or deletes went through untouched in that same session, so the heuristic is not a blanket ban on the pattern — it weighs what the process then does, and reconfiguring the network scored where adding a firewall rule did not. And **`-ExecutionPolicy Bypass` is redundant here**: execution policy governs script *files*, never `-Command` or `-EncodedCommand`, so the flag buys nothing and contributes half the signature. `-EncodedCommand` itself is load-bearing and stays — it is what keeps the nested scripts free of quoting bugs.
